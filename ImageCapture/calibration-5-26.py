...
def undistort(h, w, matrix, distortion, imageTitle):
    ...
    original_photo = Image.open(imageTitle)

    # Read the metadata from the original photo
    original_metadata = original_photo.info

    # Perform image distortion and save it as a new photo
    cv2.imwrite(newPathway, distortion)

    # Load the new photo
    new_photo = Image.open(newPathway)

    # Set the metadata of the new photo to the metadata of the original photo
    new_photo.info = original_metadata

    # Save the new photo with the retained metadata
    new_photo.save(newPathway)

    # Close the photo objects
    ...
    # original_photo = Image.open(imageTitle)
    # original_metadata = original_photo.info
    # original_photo.save(newPathway, **original_metadata)
    #
    # original_photo.close()
